...
8. Commit & push: run `git status -sb` and `git diff --name-status`, then commit the slice with a narrow message only when the slice is green and Claude reports no actionable findings. Do not batch unrelated catalog, image, domain, or package-manager changes. Push to `fork/typescript` immediately after committing — don't let commits pile up locally.
