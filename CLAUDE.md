...
7. Re-review: send the revised diff back when the first review found correctness or type-safety issues. Skip only for typo-level feedback.
8. Commit & push: run `git status -sb` and `git diff --name-status`, then commit the slice with a narrow message. Do not batch unrelated catalog, image, domain, or package-manager changes. Once the slice is green, push to `fork/typescript` immediately — don't let commits pile up locally.
